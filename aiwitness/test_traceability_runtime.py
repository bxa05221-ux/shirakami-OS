...
def test_runtime_execution_forms_machine_checkable_provenance_chain() -> None:
    ...
    result = api.execute(
        ...
        objective="Validate runtime provenance",
        protocol_ids=("traceability.example",),
        verification_scope=("runtime provenance",),
    )
...
def test_runtime_provenance_chain_remains_aligned_after_verification() -> None:
    ...
    result = api.execute(
        ...
        handoff_id="SH-HO-20260925-001",
        project="Shirakami Project",
        objective="Verify provenance revision",
        protocol_ids=("traceability.example",),
    ...
    checked = api.verify_execution(
        result["execution_id"],
        expected_transition_kind="traceability.example",
    )
    ...
    assert trace["verification_status"] == witness["verification_status"]
