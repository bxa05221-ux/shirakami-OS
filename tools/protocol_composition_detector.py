...
def list_values(text: str, section: str) -> list[str]:
    ...
    if not m: return []
    return [x.strip()[2:].strip().strip("\"\'") for x in m.group(1).splitlines() if x.strip().startswith("- ")]
...
def detect(path: Path) -> dict[str, object]:
    ...
    flows = [x.strip()[2:].strip() for x in flow.group(1).splitlines() if x.strip().startswith("- ")] if flow else []
    return {"file": str(path), "title": scalar(text, "title") or scalar(text, "name"), "version": scalar(text, "version"), "inputs": inputs, "outputs": outputs, "route": routes or flows, "signals": {"has_input": bool(inputs), "has_output": bool(outputs), "has_route": bool(routes or flows), "has_evidence": "evidence" in text.lower(), "has_landscape": "landscape" in text.lower()}}
...
def main() -> None:
    ...
    for item in results:
        s = item["signals"]
        print("| `{}` | {} | {} | {} | {} | {} |".format(item["file"], "yes" if s["has_input"] else "-", "yes" if s["has_output"] else "-", ", ".join(item["route"]) if item["route"] else "-", "yes" if s["has_evidence"] else "-", "yes" if s["has_landscape"] else "-"))
