...
def test_checkpoint_plus_tail_reconstructs_same_observable_state():
    ...
    checkpoint = LandscapeState.empty()
    checkpoint.apply_evidence(records[0])
    checkpoint.apply_evidence(records[1])
...
def test_checkpoint_replay_does_not_import_external_evidence_history():
