...
def test_aats_preserves_future_persona_extension_point():
    thread = Thread("t1").with_participant(Participant("名無し001", persona={"future": "opaque"})).with_post(
        Post("名無し001", "まず現在地を見よう", "( ˘ω˘ )")
    )
...
def test_renzan_collects_viewpoints_without_interpretation():
    thread = (Thread("t1").with_participant(Participant("名無し001")).with_participant(Participant("名無し002"))
...
def test_kasen_reexpresses_viewpoints_as_flow():
    thread = (Thread("t1").with_participant(Participant("名無し001")).with_participant(Participant("名無し002"))
        .with_post(Post("名無し001", "休んだほうがいい"))
        .with_post(Post("名無し002", "でも期限もある")))
    narrative = Kasen().compose(Renzan().collect(thread))
    assert narrative == "休んだほうがいい 見方によっては、でも期限もあるとも考えられるかもしれません。"
    assert "名無し001" not in narrative and "名無し002" not in narrative


def test_entry_path_maps_landscape_before_small_step():
    thread = Thread("t1").with_participant(Participant("名無し001")).with_post(Post("名無し001", "今は何から手を付ける？"))
