...
from runtime.protocol_api import build_default_protocol_request, register_temporary_matome
...
def test_default_request_uses_common_request_boundary():
    registry = ProtocolRegistry()
...
def test_default_cannot_be_removed_or_archived():
    registry = ProtocolRegistry()
    entry = bootstrap_default_protocol(registry)
...
def test_stabilized_matome_becomes_temporary_protocol():
    registry = ProtocolRegistry()
    bootstrap_default_protocol(registry)
...
def test_temporary_cannot_replace_default():
    registry = ProtocolRegistry()
    bootstrap_default_protocol(registry)
    with pytest.raises(ProtocolRegistryError):
