...
def test_multi_account_landscape_preserves_account_provenance():
    result_a = Runtime().execute("example.protocol", example_protocol, {"message": "account-a"})
    evidence_a = capture_evidence(result_a)

    result_b = Runtime().execute("example.protocol", example_protocol, {"message": "account-b"})
