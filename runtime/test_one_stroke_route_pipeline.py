...
def test_selected_route_runs_once_and_verifies_to_evidence():
    ...
    result = pipeline.execute(
        {"a": _protocol("a", "A"), "b": _protocol("b", "B"), "c": _protocol("c", "C")},
        {"value": ""},
    )
...
def test_generated_candidate_flows_through_human_gate_runtime_and_evidence(tmp_path):
    ...
    selection = pipeline.select_candidate("route.generated", generated[0], reviewer="human", approved=True)
    result = pipeline.execute(
        {str(a): _protocol("a", "A"), str(b): _protocol("b", "B")},
        {"value": ""},
    )
...
def test_route_candidate_enters_evolution_loop_human_review_before_approval():
    pipeline = OneStrokeRoutePipeline()
    selection = pipeline.prepare_candidate("route.loop", ("a", "b", "c"), reviewer="human")
    assert selection.candidate == ("a", "b", "c")
    assert pipeline.runtime.loop.state.value == "HUMAN_REVIEW"
    assert not any(record.transition_kind == "route.route.loop" for record in pipeline.runtime.store.all())


def test_evolution_loop_approval_then_one_stroke_execution_and_evidence():
    pipeline = OneStrokeRoutePipeline()
    pipeline.prepare_candidate("route.loop", ("a", "b", "c"), reviewer="human")
    ...
    assert result.execution.transition.data["route"] == ["a", "b", "c"]
    assert any(record.transition_kind == "route.route.loop" for record in result.evidence)
    assert any(record.signals == ("HUMAN_DECISION",) for record in pipeline.runtime.store.all())
...
def test_evolution_loop_mismatch_does_not_advance_to_accepted():
    ...
    verification = pipeline.runtime.verify(result, expected_transition_kind="route.expected", diff_ref="route.loop")

    assert verification.status == "mismatch"
    assert pipeline.runtime.loop.state.value == "DIFF"
    assert not any(
        record.transition_kind == "route.route.loop" and record.status == "completed"
...
def test_evidence_derives_structural_candidates_without_authorization(tmp_path):
    ...
    evidence = (
        ...
        EvidenceRecord(protocol_id="observed.b", status="observed", transition_kind="protocol.observed",
                       transition_data={"protocol_path": str(b)}, signals=("PROTOCOL_ARTIFACT",)),
    )
...
def test_evidence_candidate_provenance_survives_human_gate(tmp_path):
    ...
    pipeline = OneStrokeRoutePipeline()
    proposal = pipeline.propose_candidate_handoffs_from_evidence(evidence, n=2)[0]
    assert proposal.candidate == (str(a), str(b))
    assert proposal.provenance == ("observed.a", "observed.b")
    ...
    assert approved.provenance == selection.provenance
    assert pipeline._approval is not None
    assert pipeline._approval.provenance == ("observed.a", "observed.b")
...
def test_evidence_candidate_requires_explicit_human_gate(tmp_path):
    ...
    pipeline.prepare_candidate("route.evidence", candidate)
