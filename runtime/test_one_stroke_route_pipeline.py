...
def test_evolution_loop_approval_then_one_stroke_execution_and_evidence():
    ...
    human_decisions = [record for record in pipeline.runtime.store.all() if record.signals == ("HUMAN_DECISION",)]
...
def test_execution_requires_explicit_approval_envelope():
    pipeline = OneStrokeRoutePipeline()
    pipeline.prepare_candidate("route.envelope", ("a", "b"), reviewer="human")
    pipeline.approve_candidate(approved=True)

    pipeline._approval = None
    try:
        pipeline.execute({"a": _protocol("a", "A"), "b": _protocol("b", "B")})
    except RuntimeError as exc:
        assert "approval envelope" in str(exc)
    else:
        raise AssertionError("execution must fail closed without approval envelope")
