...
from runtime.background_runner import (
    BackgroundRunner,
    BackgroundRunnerError,
)
...
def test_runner_consumes_only_released_activation():
    ...
    result = BackgroundRunner().run(release, runtime, protocol)
...
def test_runner_supports_bounded_iterations():
    ...
    def protocol(context):
        return Transition(kind="runner.loop", data={"iteration": context.input})

    result = BackgroundRunner().run(release, runtime, protocol, iterations=3)
...
def test_runner_preserves_protocol_identity():
    ...
    def protocol(context):
        return Transition(kind="runner.identity", data={})

    result = BackgroundRunner().run(release, runtime, protocol, iterations=2)
