...
The Runner consumes a released Activation Pump event and maintains a bounded
execution context for repeated observable cycles. It does not approve,
promote, select, schedule, or broaden authority.
"""
    ...
    def run(
        ...
        *,
        ...
        if release.status != "released":
            raise BackgroundRunnerError("Runner requires a released Pump event")
        ...
        for iteration in range(1, iterations + 1):
            try:
                runtime.bind_activated_execution(release.protocol_id)
            ...
            except (PumpCycleError, RuntimeError) as exc:
