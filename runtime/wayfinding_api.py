...
def execute_wayfinding_request(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    Semantic selection is deliberately outside this API: the caller supplies
    the Small Step and its observable Transition data.
    """
    ...
    transition = Transition(
        ...
        data=dict(payload.get("transition", {}).get("data", {"changed": True})),
    )

    result = run_aats_wayfinding(
        thread,
        state,
        _RequestSelector(step),
        _RequestApplier(transition),
        _StateObserver(),
    )
    return {
...
def serve(host: str = "127.0.0.1", port: int = 8080) -> None:
    """Run the framework-independent local API server."""
